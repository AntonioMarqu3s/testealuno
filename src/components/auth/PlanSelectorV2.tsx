      ...
      try {
        ...
        if (error) {
          ...
          if (planosError) {
            ...
            // Aplicar filtragem também para DEFAULT_PLANS
            const filteredDefault = promoApplied 
              ? DEFAULT_PLANS 
              : DEFAULT_PLANS.filter(p => p.type !== 0 && Number(p.price) > 0);
            setPlanos(filteredDefault);
            ...
            // Aplicar filtragem aqui também
            const filteredData = promoApplied
              ? formattedData
              : formattedData.filter(p => p.type !== 0 && Number(p.price) > 0);
          ...
          const availablePlans = promoApplied 
            ? data 
            : data.filter(p => p.type !== 0 && Number(p.price) > 0);
          setPlanos(availablePlans);
        } else {
          // Mesmo se usar os planos padrão, deve aplicar a filtragem
          const availablePlans = promoApplied
            ? DEFAULT_PLANS
            : DEFAULT_PLANS.filter(p => p.type !== 0 && Number(p.price) > 0);
          setPlanos(availablePlans);
